...
public class Function
{
    ...
    ///     The built-in, autogenerated processes that if there is just one input, it returns it
    ///     unchanged.
    /// </param>
    ...
    [
        new FunctionDelegate2In<Number, Number>(inputs =>
        {
            ...
            PronumeralCollection totalPronumerals = t1.Pronumerals;
            totalPronumerals.AddRange(t2.Pronumerals);
            Dictionary<Pronumeral, int> pronumeralDictionary = new();

            foreach ((Pronumeral, int) pronumeral in totalPronumerals)
                if (pronumeralDictionary.ContainsKey(pronumeral.Item1))
                    pronumeralDictionary[pronumeral.Item1] += pronumeral.Item2;
                else
                    pronumeralDictionary[pronumeral.Item1] = pronumeral.Item2;

            PronumeralCollection resultPronumerals = new();
            foreach (KeyValuePair<Pronumeral, int> kvp in pronumeralDictionary)
                resultPronumerals.Add((kvp.Key, kvp.Value));
