...
public class Number : IExpressionValue
{
    ...
    ///     The data of the number, example: 10cm Length b10 (numberUnit Type bBase)
    /// </param>
    public static Number Parse(string numberData)
    {
        if (numberData.Length == 0) throw new ArgumentException("Please provide a valid number data string.");
        string pattern = @"^(-?\d+(\.\d+)?)(\S*)";
        Match match = Regex.Match(numberData, pattern);
        if (!match.Success) throw new ArgumentException("Please provide a valid number data string.");
        Number output = new();
    ...
    public dynamic Get(bool numericValue = true, bool unit = false, bool type = false, bool scientific = false)
    {
        ...
        string outputString = "";
        if (!scientific) {
            if (numericValue) outputString += NumericValue;
            if (unit) outputString += Unit.Suffix;
            if (type) outputString += " " + MeasurementType;
        }
        else {
            // Scientific
            outputString = ToScientificNotation(NumericValue);
            if (unit) outputString += Unit.ToString();
            if (type) outputString += " " + MeasurementType;
        }
    ...
    {
        if (MeasurementType != targetUnit.MeasurementType)
    ...
    {
        if (a.MeasurementType != b.MeasurementType) return false;
        Number bConverted = b.Convert(a.Unit);
        return Math.Abs(a.NumericValue - bConverted.NumericValue) < double.Epsilon;
    }
    ...
    /// <summary>
    ///     Converts the number to a term
    /// </summary>
    /// <param name="n"> </param>
    /// <returns> </returns>
    public static implicit operator ExpressionTerm(Number n)
    {
        return new ExpressionTerm(n, [Pronumeral.NoPronumeral]);
    }
