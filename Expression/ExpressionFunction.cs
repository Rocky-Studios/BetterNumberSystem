...
{
    /// <summary>
    ///     The full name of the function e.g. Sum, Integral.
    /// </summary>
    public readonly string Name;

    /// <summary>
    ///     A short and recognisable symbol used to identify this function e.g. + for sum, ∫ for integral.
    /// </summary>
    ...
    /// </summary>
    public readonly MathFunction Function;

    /// <summary>
    ///     The inputs to the computation.
    /// </summary>
    public readonly ExpressionFunctionInputs Inputs;
...
public class ExpressionFunctionInputs
{
    ...
    /// <summary>
    ///     Creates a new function input declaration by specifying the input amounts
    /// </summary>
...
public struct ExpressionFunctionInputAmount(bool one, bool two, bool three, bool infinite)
{
    ...
    /// <summary>
    ///     Where this function works when it has any amount of inputs.
    /// </summary>
...
public static class FunctionManager
{
    ...
    /// </summary>
    /// <param name="name"> The full name of the function e.g. Sum for +. </param>
    /// <returns> </returns>
    public static ExpressionFunction Get(string name)
    {
        try {
            ...
            if (returnedFunction is null) throw new ArgumentException("Cannot find function with name " + name);
    ...
    public static ExpressionFunction Get(string name, ExpressionGroup[] inputs)
    {
        ...
        returnedFunction.Inputs.Inputs =
            inputs.Select(input => new ExpressionFunctionInput(null) { Value = input }).ToList();
        return returnedFunction;
    }
    ...
    static FunctionManager()
    {
        ...
        _ = new ExpressionFunction("Sum", "+",
            inputs =>
            {
                ...
                foreach (KeyValuePair<List<Pronumeral>, List<ExpressionTerm>> likeTerm in input.ToLikeTermsCollection())
                ...
                foreach (ExpressionTerm expressionTerm in likeTermCollection.Value)
                    if (expressionTerm.Value is not Number)
        ...
        _ = new ExpressionFunction("Difference", "-",
            inputs =>
            {
                switch (inputs.Inputs.Count) {
                    ...
                    {
                        LikeTermsCollection likeTerms = [];
                        foreach (KeyValuePair<List<Pronumeral>, List<ExpressionTerm>> likeTerm in inputs.Inputs[0]
                                     .ToLikeTermsCollection())
                            if (likeTerms.ContainsKey(likeTerm.Key)) {
                                List<ExpressionTerm> terms = likeTerms[likeTerm.Key];
                                terms.AddRange(likeTerm.Value);
                                likeTerms[likeTerm.Key] = terms;
                        ...
                        foreach (KeyValuePair<List<Pronumeral>, List<ExpressionTerm>> likeTerm in likeTerms)
                        foreach (ExpressionTerm term in likeTerm.Value)
                            switch (term.Value) {
                                case Number n:
                                    n.NumericValue *= -1;
                                    break;
                    ...
                    case 2:
                        ...
                        foreach (KeyValuePair<List<Pronumeral>, List<ExpressionTerm>> likeTerm in inputs.Inputs[1]
                                     .ToLikeTermsCollection())
                        ...
                        if (
                            likeTermsA.SelectMany(likeTermCollection
                                    => likeTermCollection.Value)
                                .Any(expressionTerm => expressionTerm.Value is not Number) ||
                        ...
                        foreach (KeyValuePair<List<Pronumeral>, List<ExpressionTerm>> likeTermCollection in
                                 likeTermsA) {
                            List<Pronumeral> pronumerals = likeTermCollection.Key;
                            Number total = new(0, (likeTermCollection.Value[0].Value as Number)!.Unit);
                    ...
                    default:
                        throw new ArgumentOutOfRangeException(nameof(inputs),
                            "Difference function can only take 1 or 2 inputs");
                }
            },
            new ExpressionFunctionInputs(true, true)
        );
        ...
        _ = new ExpressionFunction("Sine", "sin",
            inputs =>
            {
                ...
                if ((likeTerms.First().Value[0].Value as Number)!.MeasurementType != MeasurementType.Angle)
                    throw new ArgumentException("Sine function only works with angles");
                ...
                foreach (KeyValuePair<List<Pronumeral>, List<ExpressionTerm>> likeTermCollection in likeTerms) {
                    ...
                    output.Add(pronumerals,
                        [(ExpressionTerm)new Number(Math.Sin(angleInRads), UnitManager.Units["Plain"])]);
                }

                return output;
            },
            new ExpressionFunctionInputs(true)
        );
        ...
        _ = new ExpressionFunction("Cosine", "cos",
            inputs =>
            {
                ...
                foreach (KeyValuePair<List<Pronumeral>, List<ExpressionTerm>> likeTermCollection in likeTerms) {
                    ...
                    output.Add(pronumerals,
                        [(ExpressionTerm)new Number(Math.Cos(angleInRads), UnitManager.Units["Plain"])]);
                }
